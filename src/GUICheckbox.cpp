...
void GUICheckbox::render() {
	GUIRenderer::drawRect(m_shadowRect, ColorRGBA8(0, 0, 0, 128)); // Rendering shadow
	GUIRenderer::drawRect(glm::vec4(m_destRect.x - 1, m_destRect.y - 1, m_destRect.z + 2, m_destRect.w + 2), ColorRGBA8(20, 20, 20, 255)); // Rendering outline
	GUIRenderer::drawRect(m_destRect, m_currentColor); // Rendering on/off rect
}
